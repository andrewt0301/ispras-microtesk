...
final class IrTranslator {
  ...
  private static final class ArrayAccess {
    public final NodeVariable array;
    public final NodeVariable index;
    public final NodeVariable value;

    public static ArrayAccess load(final NodeVariable array) {
      return new ArrayAccess(array, "read");
    }

    public static ArrayAccess store(final NodeVariable array) {
      return new ArrayAccess(array, "write");
    }

    private ArrayAccess(final NodeVariable array, final String access) {
      final DataType type = array.getDataType();

      this.array = array;
      this.index = new NodeVariable(
        String.format("%s.%s.index", array.getName(), access),
        (DataType) type.getAttribute(DataTypeId.Attribute.KEY));

      this.value = new NodeVariable(
        String.format("%s.%s.value", array.getName(), access),
  ...
  private static Node createRValue(LValue lvalue, boolean signal) {
    ...
    if (lvalue.isMacro()) {
      ...
      root = new NodeVariable(lvalue.base);
    }
  ...
  private LValue[] fetchConcatLValues(final List<Node> values) {
    ...
    for (int i = 0; i < lhs.length; ++i) {
      lhs[i] = createLValue(locationFromNodeVariable(values.get(i)), prefix);
    }
    return lhs;
  }
  ...
  private void assignToAtom(Location lhs, Node value) {
    final LValue lvalue = createLValue(lhs, prefix);
    if (lvalue.isArray()) {
  ...
  private static LValue createLValue(Location atom, final String prefix) {
    ...
    case MEMORY: // FIXME recursive processing required
      ...
      if (alias != null) {
        return createLValue((Location) alias.getLocation(), prefix);
      }
      break;
    ...
    if (atom.getIndex() != null) {
      ...
      if (size > 0) {
        index = convertExpression(atom.getIndex().getNode(), prefix);
    ...
    if (atom.getBitfield() != null) {
      final Node major = convertExpression(atom.getBitfield().getFrom().getNode(), prefix);
      final Node minor = convertExpression(atom.getBitfield().getTo().getNode(), prefix);
  ...
  private Node convertExpression(final Node node) {
    return convertExpression(node, this.prefix);
  }
   ...
   */
  private static Node convertExpression(Node expression, final String prefix) {
  ...
  private NodeOperation createOutput(final Node node) {
    final Data data;
    if (node instanceof NodeValue) {
      data = ((NodeValue) node).getData();
    } else {
      data = node.getDataType().valueUninitialized();
    }
